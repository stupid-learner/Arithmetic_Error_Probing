  ...
  def __init__(
    ...
    if model is None:
      ...
      model = transformers.AutoModelForCausalLM.from_pretrained(
          ...
          quantization_config= bnb_4bit_config if use_4bit else None,
          device_map='auto'
          )
...
def remove_space_tokens(tokens):
  """Remove space tokens from a list of tokens."""
  space_token_id = 220
  return [t for t in tokens if t != space_token_id]
...
def make_inputs(tokenizer, prompts, device="cuda"):
  ...
  #token_lists = [remove_space_tokens(t) for t in token_lists]
